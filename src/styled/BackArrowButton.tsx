...
const BackArrowButton = forwardRef<HTMLButtonElement, Props>(
  ({ children, color, href, isSmall, ...props }, ref: any) => {
    return (
      <button
        className={`group inline-flex items-center gap-2 text-primary transition duration-200 disabled:cursor-not-allowed disabled:text-gray-300 group-hover:font-medium group-hover:text-gray-300 md:items-center ${
          !isSmall && "lg:text-lg"
        }`}
        {...props}
        ref={ref}
